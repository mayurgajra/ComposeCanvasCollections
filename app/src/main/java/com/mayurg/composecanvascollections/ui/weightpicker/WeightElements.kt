        ...
        for (i in minWeight..maxWeight) {
            ...
            drawContext.canvas.nativeCanvas.apply {
                if (lineType is LineType.TenStep) {
                    val textRad = (outerRadius - lineLength - 5.dp.toPx() - style.textSize.toPx())

                    val x = textRad * cos(angleInRad) + circleCenter.x
                    val y = textRad * sin(angleInRad) + circleCenter.y
                    withRotation(
